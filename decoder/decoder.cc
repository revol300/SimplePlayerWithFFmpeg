...
void Decoder::init() {
  ...
  auto codec = avcodec_find_decoder(fmt_ctx_->streams[stream_index_]->codecpar->codec_id);
  if(codec == NULL){
...
bool Decoder::getFrame(std::shared_ptr<AVPacket>& packet, std::shared_ptr<AVFrame>& frame) {
