...
int AudioConverter::getFrame(AVFrame *af,
                             std::shared_ptr<AudioFrame> &audio_buf) {
  int resampled_data_size = 0;
  int64_t dec_channel_layout;
  int data_size = av_samples_get_buffer_size(
      NULL, audio_codec_context_->channels, af->nb_samples,
  ...
  if (!swr_ctx_.get()) {
    ...
    dec_channel_layout =
        (af->channel_layout &&
         av_frame_get_channels(af) ==
             av_get_channel_layout_nb_channels(af->channel_layout))
            ? af->channel_layout
    ...
    if (af->format != audio_hw_params_.fmt ||
        ...
        dec_channel_layout != audio_hw_params_.channel_layout || !swr_ctx) {
      if (!swr_ctx)
        swr_free(&swr_ctx);
      swr_ctx = swr_alloc_set_opts(
          NULL, audio_codec_context_->channel_layout,
          (AVSampleFormat)AV_SAMPLE_FMT_S16, audio_hw_params_.freq,
          av_get_default_channel_layout(audio_hw_params_.channels),
          (AVSampleFormat)af->format, af->sample_rate, 0, NULL);
      ...
      swr_ctx_ = std::shared_ptr<SwrContext>(
          swr_ctx, [](SwrContext *swr_ctx) { swr_free(&swr_ctx); });
  ...
  if (swr_ctx_.get()) {
    ...
    int out_count =
        (int64_t)af->nb_samples * audio_hw_params_.freq / af->sample_rate + 256;
    int out_size = av_samples_get_buffer_size(
        NULL, audio_hw_params_.channels, out_count, audio_hw_params_.fmt, 0);
    int len2;
    if (out_size < 0) {
      av_log(NULL, AV_LOG_ERROR, "av_samples_get_buffer_size() failed\n");
      return -1;
    }
    // @NOTE : https://ffmpeg.org/doxygen/3.2/group__lswr.html#details 참고해서
    // 수정
    uint8_t *memory = reinterpret_cast<uint8_t *>(av_malloc(out_size));
    uint8_t **out = &memory;
    ...
    audio_buf = std::shared_ptr<AudioFrame>(audio_frame, [](AudioFrame *f) {
      // std::cout << "converted_buffer freed (Audio)" << std::endl;
      av_free(reinterpret_cast<void *>(f->data));
      delete f;
    });
  } else {
    //@TODO : FIXME
