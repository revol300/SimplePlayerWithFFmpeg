...
static void audio_callback(void *userdata, Uint8 * stream, int len) {
   ...
   AudioRenderer* audio_renderer = (AudioRenderer*) userdata;
   int len1,audio_size;
   ...
   audio_time = audio_renderer->getAudioTime() - audio_renderer->bytesToMilisecond(audio_bytes_len) - 2* audio_renderer->bytesToMilisecond(audio_buf_size);
...
AudioRenderer::AudioRenderer(std::shared_ptr<AVCodecContext> &audio_codec_context, AVRational time_base) : audio_codec_context_(audio_codec_context), time_base_(time_base) {}
