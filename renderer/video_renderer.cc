...
VideoRenderer::VideoRenderer(
    std::shared_ptr<AVCodecContext> &video_codec_context, AVRational time_base)
    : video_codec_context_(video_codec_context), time_base_(time_base) {}
...
bool VideoRenderer::init() {
  ...
  screen_ =
      SDL_CreateWindow("FFmpegSimplePlayer", SDL_WINDOWPOS_UNDEFINED,
                       SDL_WINDOWPOS_UNDEFINED, video_codec_context_->width,
                       video_codec_context_->height, SDL_WINDOW_RESIZABLE);
...
void VideoRenderer::runLoop() {
  ...
  while (running) {
    ...
    if (frame_queue_.size() > 0) {
      ...
      //@NOTE : time unit ms
      if ((getFrameTime(in.get()) + 200) <
          Timer::getInstance()->getAudioTime()) {
        frame_queue_.pop();
      } else if (getFrameTime(in.get()) <
                 Timer::getInstance()->getAudioTime()) {
        SDL_UpdateYUVTexture(texture_, NULL, in->data[0], in->linesize[0],
                             in->data[1], in->linesize[1], in->data[2],
                             in->linesize[2]);
